...
SYNTHESIS_PROMPT_ZH = (
    ...
    "merchant_analysis 含 topAsins 时按商家 ID、商家名称、Top ASIN 输出表格，保留各商家的原排序和归属；解释 asinRanking 的来源日期、区间与口径，空列表和失败商家逐项说明，不推断没有商品，不编造 ASIN、销量或排名。只覆盖当前快照最多五个；明确用户要求的其他期间或更多数量未覆盖。未知元数据如实标记未知。不要输出无关月度数据或月度不可用提示。\n"
...
SYNTHESIS_PROMPT_EN = (
    ...
    "When merchant_analysis includes topAsins, show merchant ID, name and Top ASINs in a table, preserving each merchant's order and ownership. Explain asinRanking snapshot dates, range and basis; report each empty or failed merchant without claiming it has no products. Never invent ASINs, sales or rankings. Only up to five from the current snapshot are covered; disclose any requested other dates or larger counts as unsupported. Mark unknown metadata as unknown. Omit unrelated monthly data and monthly-unavailable warnings.\n"
