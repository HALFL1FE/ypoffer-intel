...
bundle = hooks.currentRecommendationBundle();
assertTruthy(
  bundle.rows.length > 0 && bundle.rows.length < 100,
  "bundle should return every available row when the tier does not have enough candidates"
);
...
assertEqual(bundle.gaps[0].available, bundle.rows.length, "shortage should report the dynamic available count");
assertEqual(bundle.gaps[0].gap, 100 - bundle.rows.length, "shortage should report the dynamic missing count");
