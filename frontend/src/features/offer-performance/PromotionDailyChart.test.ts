...
describe("Daily chart", () => {
  it("shows a single chronological series, actual dates, metric switching and pending dates", async () => {
    ...
    expect(wrapper.findAll("circle.after")).toHaveLength(9);
    expect(wrapper.findAll(".promotion-trend-line")).toHaveLength(1);
    expect(wrapper.findAll("tbody tr")[9]!.text()).toContain("暂无数据");
    expect(wrapper.findAll("tbody tr")[0]!.text()).toContain("2026-09-04");
    ...
    expect(wrapper.findAll("tbody tr")[0]!.text()).toContain("2026-08-28");
    expect(wrapper.findAll("tbody tr")[14]!.text()).toContain("US$30.00");
