...
### 4.5 对话记忆从文本历史升级为结构化状态（首期已完成）

原实现只在当前页面运行时保留 Agent 成功问答文本，刷新页面后丢失；将整段文本继续传给模型也容易受字符截断影响。本次首期实现改为保存可控的结构化状态，完整对话、原始问题、回答正文和工具明细仍不进入持久化记忆。

已实现：

- 新增 `public/agent_memory_state.js`，使用 `oi_agent_memory_v1` 保存版本化状态：最近商户 ID/标准名称、品类、Tier、月份范围、指标名、最近工具摘要、数据来源、快照时间和部分执行标识。
- `public/app.js` 从成功工具结果生成安全事件；同一轮多个工具结果合并，下一轮成功查询替换上一轮焦点，避免跨轮商户集合无限增长。
- 歧义商户只保存候选实体及其 `pending/confirmed/rejected` 状态，不保存原始错误、原始问题或工具参数。
- 状态仅保留白名单字段和长度上限；不保存指标数值、月度行、付款行、工具 JSON、模型消息、回答正文或异常堆栈。
- 状态写入 `localStorage`，版本不匹配、超过 7 天、未来时间戳、超长内容或存储异常都会安全清除/降级为空状态。
- Agent 页面刷新后恢复结构化上下文，并在规划请求中以受控文本传递；当前数值仍要求重新调用数据工具。欢迎区会显示已恢复上下文；“新对话”和登出都会清除内存与持久化状态。
- 本次不新增数据库字段、表、API 路由或后端存储；Report Mode 原有报告记忆快照保持原契约不变。

本阶段仍未实现：会话列表、跨设备同步、分享、历史消息完整恢复和独立会话删除。后续实现这些能力时，仍应沿用白名单投影和敏感内容不落盘约束。
...
- 已完成首期：Agent 页面结构化多轮记忆、刷新恢复、7 天过期和新对话/登出清除。
- 待实施：会话列表、跨设备保存、恢复、删除和分享。
- 失败重试、问题编辑、复制和导出。
- 完善可访问性、移动端和中英双语。

验收标准：刷新或长对话后，Agent 仍能准确理解当前商户、指标和时间范围；持久化状态不包含原始问题、回答正文、工具明细或异常堆栈；用户可以复用或导出结果。
