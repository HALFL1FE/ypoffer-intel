...
> 当前状态：审查结论与实施建议；截至 2026-08-24，P0.1 综合请求读取上限、P0.2 工具调用批处理和 P0.3 可验证数据来源边界已完成实现并通过定向回归测试
...
### 3.4 统一实体和过滤条件的失败关闭策略

当前商户严格解析最后仍允许名称子串匹配并返回第一项。付款状态、月份等过滤条件无法识别时，也可能被忽略，导致查询范围扩大。
...
同时应让趋势、商户分析、商户对比和付款查询复用同一套商户解析逻辑，确保“ID 优先”规则一致。
...
本次已验收：6 个工具调用按 4+2 批次完整执行；超过总预算的目标带有 partial/omittedTargets 标识并进入回答提示；综合流入口实际读取上限与 128KB 配置一致。Phase 1 其余边界项仍待后续实施。
...
本次新增回归结果：Agent 测试共 27 个场景通过；6 个工具调用按 4+2 批次完整执行；7 个工具调用执行 6 个并明确返回 1 个 `omittedTargets`。综合请求配置测试同时确认规划上限为 64KB、综合上限为 128KB，且两个综合入口均传入共享读取上限。
