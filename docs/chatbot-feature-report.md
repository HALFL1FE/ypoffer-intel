...
> **Top ASIN 收入与分表（2026-09-22）：** 排名快照版本 3 保留 `topAsinMetrics`，收入与 Top ASIN 使用同一次查询、同一商家和同一日期区间。Agent 首次回答、结构化表和综合失败兜底均显示期间收入；零收入保留为零，缺少可查询字段时标记未提供。旧快照在接口读取时重建。多个 ASIN 的商品详情及月度表现按 ASIN 拆成独立表格，仍保留商家归属和追问上下文。

> **Agent 商户完整月度展示（2026-09-20）：** 普通 `merchant_analysis` 与 Deep Window 一样请求完整商户接口（`months=12&limit=50`，不使用 `minimal=1`，超时 30 秒），复用 Report 的 Amazon/aggregate 月份合并逻辑；同月 aggregate 字段覆盖 Amazon 字段。商户详情补充当前快照，比较指标仍来自快照。Agent 直接渲染月度表，缺失指标显示未提供；仅有月份不代表指标完整。修复月份解析把 10–12 月误读为 1 月的问题。趋势工具仍保留原精简查询路径，Top ASIN 仍使用现有快照。

> **Agent ASIN 追问上下文：** 上一轮已完成工具结果中的 ASIN 按原顺序传入下一轮规划，支持“这几个 ASIN”“第一个”等引用。只传递最多 30 个合法编号，服务端按不可信引用数据校验；每次详情工具仍最多 5 个。Modern session 与 CopilotKit/AG-UI 均接入，结构化记忆不持久化 ASIN，新对话清空引用，当前明确输入的 ASIN 优先。

> **ASIN 价格格式兼容：** 商品价格和原价保留数据库返回的货币标记（如 `$51.99`、`$1,299.00`），同时支持数值价格；空值及无法识别的价格显示未提供，不补零、不推断币种。结构化结果与综合失败兜底使用同一价格投影。

> **Agent ASIN 商品详情：** `asin_analysis` 增加可选 `view=details/performance`。商品详情或裸 ASIN 查询使用 details；业绩和月度请求使用 performance，旧调用仍兼容。详情按字段展示商家 ID/名称、商品名称、链接、品类、价格、原价、折扣、佣金率源值与商品更新时间，缺失字段显示未提供，不推断币种或佣金率单位。商品资料不依赖月度业绩，有资料无业绩时不再补造零指标。商品更新时间与接口查询时间分别保留；综合失败仍展示已完成的详情结果。数据范围限于现有数据库字段，不额外抓取 Amazon 页面。
